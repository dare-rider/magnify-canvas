...
class Experience {
  ...
  bind() {
    ...
    this.canvas.addEventListener("mousedown", this.onMouseClicked.bind(this));
  }
  ...
  render() {
    ...
    // This is needed again as to render the marker in magnified context
    this.drawMarkers();
  ...
  onMouseClicked(mouse){
    var rect = this.canvas.getBoundingClientRect();
    var mouseXPos = ((mouse.x - 7) - rect.left);
    var mouseYPos = ((mouse.y - 7) - rect.top);

    // Move the marker when placed to a better location
    var marker = new Marker();
    marker.XPos = mouseXPos - (marker.Width / 2);
    marker.YPos = mouseYPos - marker.Height;
    var markerXKey = (Math.round(marker.XPos/20) * 20);
    var markerYKey = (Math.round(marker.YPos/40) * 40);

    let markerKey = `${markerXKey}-${markerYKey}`
  ...
  drawMarkers() {
    for (const markerKey in this.markers) {
      var tempMarker = this.markers[markerKey];
      // Draw marker
